...
PUBLIC_URL = "https://kiruba-dicom.is-a.dev"
...
def cstore(event):
    try:
        ...
        print("Saved:", path)
        print("SECURE LINK:", f"{PUBLIC_URL}/p/{token}")
...
def dicom_meta(t, series_uid, name):
    l = getlink(t)
    if not l or not authed(t):
        abort(403)

    c = db()
    r = c.execute(
        "SELECT folder FROM series WHERE uid=? AND study_uid=?",
        (series_uid, l["uid"])
    ).fetchone()
    c.close()
    ...
    try:
        ...
        def val(tag, default=None):
            try:
                x = getattr(ds, tag, default)
                if x is None:
                    return default
                if hasattr(x, "VM") and len(x) == 1:
                    return x[0]
                return x
            except Exception:
                return default
        ...
        return jsonify({
            "rows": int(val("Rows", 0) or 0),
            "columns": int(val("Columns", 0) or 0),

            "pixelSpacing": nums("PixelSpacing"),
            "sliceThickness": float(val("SliceThickness", 0) or 0),
            "spacingBetweenSlices": float(val("SpacingBetweenSlices", 0) or 0),

            "rescaleSlope": float(val("RescaleSlope", 1) or 1),
            "rescaleIntercept": float(val("RescaleIntercept", 0) or 0),

            "windowCenter": nums("WindowCenter"),
            "windowWidth": nums("WindowWidth"),

            "imagePositionPatient": nums("ImagePositionPatient"),
            "imageOrientationPatient": nums("ImageOrientationPatient"),

            "instanceNumber": int(val("InstanceNumber", 0) or 0),
            "sopInstanceUID": str(val("SOPInstanceUID", "") or ""),
            "seriesInstanceUID": str(val("SeriesInstanceUID", "") or ""),
            "studyInstanceUID": str(val("StudyInstanceUID", "") or ""),

            "modality": str(val("Modality", "") or ""),
            "bodyPart": str(val("BodyPartExamined", "") or ""),
            "manufacturer": str(val("Manufacturer", "") or "")
        })

    except Exception as e:
        return jsonify({
            "error": "Unable to read DICOM metadata",
            "detail": str(e)
        }), 500
